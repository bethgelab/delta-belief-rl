...
def per_turn_reward_fn(
    ...
    if "sorry, i cannot answer this question" in obs.lower() or action.count("?") > 1:
        reward_penalty += signals["invalid_action"]
        invalid = True

    # this will not play a role in the current implementation
    ...
    for hist_dict in history[1:]:
        ...
        if isinstance(hist_dict, dict):
            if hist_dict["role"] == "assistant":
                ...
                content = hist_dict.get("content", "")
                obs_list.append(content)
        # otherwise exit the loop
        else:
            break

    if len(action_list) != len(obs_list):
        print(f"[DEBUG] action_list ({len(action_list)}): {action_list}")
        print(f"[DEBUG] obs_list ({len(obs_list)}): {obs_list}")
        print(f"[DEBUG] history: {history}")
        raise AssertionError(
            f"(TRAJ-REWARD-FN) Number of parsed actions ({len(action_list)}) and observations ({len(obs_list)}) must match"
        )
